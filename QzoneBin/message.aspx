<%@ Page Language="C#" AutoEventWireup="true" CodeFile="message.aspx.cs" Inherits="comment" %>
...
<body>
    ...
    <div>
        ...
        <div>
                ...
                <!--main-->
            <asp:Button ID="bntHostMess" runat ="server" Text ="编辑主人寄语" OnClick="bntHostMess_Click" />  &nbsp&nbsp&nbsp&nbsp
            <asp:Button ID="bntSubmitMess" runat ="server"  Text ="保存寄语" OnClick="bntSubmitMess_Click" />
            <br />
            <asp:TextBox ID="txtHosthope" runat ="server" Enabled ="false" TextMode ="MultiLine" MaxLength="200"  Height ="100px" Width="400px" ></asp:TextBox>
            <br />
            ...
            <asp:Repeater ID="rptFist" runat ="server" OnItemCommand="rptFist_ItemCommand" OnItemDataBound="rptFist_ItemDataBound"   >
                      ...
                      <ItemTemplate >
                          <tr>
                              <td><asp:ImageButton ID="imgbtnHostHead" runat ="server" ImageUrl='<%# Eval("Uheadimg") %>'   PostBackUrl='<%# "message.aspx?uqq=" + Eval("Mqq") %>'   Width="50px" Height="50px"  CommandName="imgHeadFirst" /> <asp:Label ID="lblHostNike" runat ="server" Text ='<%# Eval("Unick") %>'></asp:Label></td>
                              <td><asp:Label ID="lblSayContent" runat="server" Text ='<%# Eval("Mcontent") %>' Font-Size="Large"></asp:Label></td>
                               <td>  <asp:Button ID ="btnSayReply" runat="server" Text ="回复"  CommandName="FirstReply" CommandArgument='<%# Eval("Mid") %>'/></td>
                                <td><asp:Button ID="btnDelFirst" runat ="server" Text ="删除" CommandName="DelFirst"  CommandArgument='<%# Eval("Mid") %>' /></td>
                          </tr>

                          <tr>
                              <td>
                                  <asp:Label ID="lblFirstTime"  runat ="server" Text='<%# Eval("MpublishTime") %>'></asp:Label>
                                  </td>
                          ...
                          <asp:Repeater ID="rptSeond" runat="server" OnItemCommand="rptSeond_ItemCommand" >
                              ...
                              <ItemTemplate>
                                 <tr>
                                   <td><asp:ImageButton ID="imgbtnCommenterHead" runat ="server" ImageUrl='<%# Eval("Uheadimg") %>' PostBackUrl='<%# "message.aspx?uqq=" + Eval("MCguestqq") %>'  CommandName="imgHeadSecond"  Width="35px" Height="35px" /> <asp:Label ID="lblHostNike" runat ="server" Text ='<%# Eval("Unick") %>'></asp:Label></td>
                              <td><asp:Label ID="lblSayComment" runat="server" Text ='<%# Eval("MCcontent") %>' Font-Size="Large"></asp:Label></td>
                             <td><asp:Button ID="btnDelSecong" runat="server" Text ="删除" CommandName="DelSecond" CommandArgument='<%# Eval("MCid") %>' /></td>
                                      </tr>
                                     </ItemTemplate>
                          ...
                          <tr>
                              ...
                              <td></td>
                             <td>  <asp:Button ID ="btnSaySub" runat="server"   Visible="false"  Text ="提交"  CommandName="FirstSubSay" CommandArgument='<%# Eval("Mid") %>'/></td>
